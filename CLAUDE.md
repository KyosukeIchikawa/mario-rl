...
- **`--rl` choices are out of sync with the dispatch.** argparse declares `choices=["categorical_dqn", "ddqn", "ddqn_per"]`, but `Main.__init__` dispatches on `categorical_dqn|ddqn|dueling_ddqn|noisy_dqn|prioritized_ddqn`. So via CLI only `categorical_dqn` and `ddqn` actually work today; `ddqn_per` is accepted but unhandled (raises `ValueError`), and the other three are implemented but rejected by argparse. When wiring up an agent, update **both** the `choices` list and the dispatch block.
